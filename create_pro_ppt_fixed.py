...
# Delete all old mixed slides
delete_all_slides(prs)

# Add 14 fresh clean slides using the template's master layout
slides = []
for _ in range(14):
...
def add_body(slide, points, top_in=1.6):
    left = Inches(0.5)
    top = Inches(top_in)
    width = Inches(12)
    height = Inches(5.5)
    ...
    for i, (text, level) in enumerate(points):
        ...
        p.text = text
        p.level = level
        p.font.size = Pt(26 - (level * 4)) # 26 for level 0, 22 for level 1
        p.font.name = 'Segoe UI'
...
p3.font.color.rgb = RGBColor(190, 190, 190)
...
prs.save("PathCompanionAI_Final_Presentation_Fixed.pptx")
print("Fixed Presentation saved successfully.")
